...
class DashboardApp:
    ...
    def _setup_callbacks(self):
        ...
        def update_graph_and_events(time_range, auto_range_trigger, refresh_trigger):
            ...
            logger.debug(f"Data metrics: {list(data.get('data', {}).keys()) if data else []}")
        ...
        def update_map(refresh_trigger):
            """Show map zoomed to node locations (OpenStreetMap embed)."""
            nodes = self.api.get_nodes()
            with_coords = [n for n in nodes if n.get("latitude") is not None and n.get("longitude") is not None]
            if not with_coords:
                return html.Div(
                    "No nodes with location. Add nodes (relay receives POST from nodes with name/lat/lon) to see map.",
            ...
            osm_url = f"https://www.openstreetmap.org/export/embed.html?bbox={bbox}&layer=mapnik&marker={sum(lats)/len(lats)}%2C{sum(lons)/len(lons)}"
            return html.Div(
                [
                    html.Iframe(
                        src=osm_url,
